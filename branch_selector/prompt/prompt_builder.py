...
5. 【不要选】如果变量很可能来源于数据库连接（不包括查询结果数据）
6. 【不要选】如果是系统环境检查（PHP版本、SQL版本、系统版本等），或者服务器环境检查（如Apache、Nginx等）
7. 【不要选】如果是本地文件存在性检查
8. 【不要选】如果是数据库连接测试或组件功能验证
9. 【不要选】如果是检查常量是否已经被定义
9. 【应该选】对于不确定来源的变量，优先考虑它是否可能来自GET、POST、COOKIE、SESSION或环境变量
10. 【应该选】如果变量名或使用模式暗示可能是用户输入（如$input、$param、$data等），选择该分支
11. 【应该选】对于处理业务数据的变量，倾向于认为可能来自用户输入
12. 【应该选】如果无法确定，但有可能性（哪怕很小）变量间接来自可修改的五种来源之一（GET、POST、COOKIE、SESSION、环境变量），就选择该分支

如果明确规则没有覆盖到的话，请选择所有可能（哪怕只是理论上可能）被外部输入影响的分支。
