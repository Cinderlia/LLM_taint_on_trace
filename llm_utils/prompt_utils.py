...
from if_extract import norm_trace_path
...
_DEFAULT_LLM_TAINT_TEMPLATE_TAIL = (
    ...
    "仅允许输出以下类型：AST_VAR、AST_PROP、AST_DIM、AST_METHOD_CALL、AST_CALL。\n"
    "类型特征参考：\n"
    "AST_METHOD_CALL：形如 obj->method(...) 或 obj.method(...)，通常带()。\n"
    "AST_CALL：形如 func(...)，通常带()。\n"
    "AST_PROP：形如 obj->prop 或 obj.prop，不带()。\n"
    "AST_DIM：形如 arr[key]。\n"
    ...
    "请同时输出数据流的有向边（dataflow edges），方向为：影响者 -> 被影响者。\n"
...
DEFAULT_LLM_TAINT_TEMPLATE_VAR = (
    ...
    "所有能够影响到{type}变量{name}的变量和函数调用"
    + _DEFAULT_LLM_TAINT_TEMPLATE_TAIL
)

DEFAULT_LLM_TAINT_TEMPLATE_FUNC = (
    "你是一个代码分析助手,请你找出下列代码中"
    "所有影响到{type}函数{name}的返回值的变量和函数调用"
    + _DEFAULT_LLM_TAINT_TEMPLATE_TAIL
)
...
def render_llm_taint_prompt(*, template: str, taint_type: str, taint_name: str, result_set: str) -> str:
    ...
    return (
        (t.replace('{type}', str(taint_type or ''))
          .replace('{name}', str(taint_name or ''))
...
def locs_to_seq_code_block(locs, ctx, *, prefer: str = 'forward'):
    ...
    return '\n'.join(s for _, _, _, s in out_lines)
...
def should_skip_llm_scope(scope_seqs, ctx) -> bool:
    if not isinstance(ctx, dict):
        return False
    cur = []
...
def map_result_set_to_source_lines(scope_root: str, result_set, trace_index_path: str = 'trace_index.json', windows_root: str = r'D:\files\witcher\app'):
...
def generate_taint_prompt(result_set_or_path, scope_root: str, base_prompt: str = '', trace_index_path: str = 'trace_index.json', windows_root: str = r'D:\files\witcher\app', taint_sources=None) -> str:
    rs = None
    ts = taint_sources
