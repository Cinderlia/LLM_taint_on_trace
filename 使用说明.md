...
### 2) `trace_utils/trace_edges.py`

用途：根据 `trace.log` 相邻 trace 分组，推断缺失的 `CALLS` 边并输出到 `trace_edges.csv`，用于弥补 `cpg_edges.csv` 中缺失的调用边。

用法：

- `python trace_utils/trace_edges.py`
  - 可带路径参数：`python trace_utils/trace_edges.py --input-dir ./input --tmp-dir ./tmp --test-dir ./test`
